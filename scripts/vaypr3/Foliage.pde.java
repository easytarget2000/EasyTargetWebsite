class Foliage {
  ...
  private int roundsPerDrawCall;

  private int NUM_OF_INITIAL_NODES;

  private int MAX_AGE;

  private int ADD_NODE_ROUND_LIMIT;

  private int MAX_NUM_OF_NODES;
  ...
  public Foliage() {
    roundsPerDrawCall = 8;
    NUM_OF_INITIAL_NODES = 128;
    MAX_AGE = 128;
    ADD_NODE_ROUND_LIMIT = 4;
    MAX_NUM_OF_NODES = 512;

    center = new PVector(width / 2f, height / 2f, 0f);
    numOfNodes = 0;
    nodeAddCounter = 0;
    displaySize = max(width, height);
    jitter = displaySize * 0.001f;
    nodeDensity = (int) (NUM_OF_INITIAL_NODES / 16f);
    stopped = false;

    age = 0;

    startX = width / 2f; // + random(width * 0.33f);
